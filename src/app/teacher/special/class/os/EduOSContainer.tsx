...
export default function EduOSContainer() {
  ...
  const getResponsiveScale = useCallback((id: WindowType) => {
    ...
    if (isMobile) {
      return { x: 10, y: 10, width: vw * 0.92, height: vh * 0.75 };
    } else {
      ...
      <div className="flex-1 relative p-2 md:p-6 overflow-hidden">
        {Object.values(windows).map((win) => (
          <Rnd
            ...
            // 🚀 [핵심 수술] 리사이즈 영역을 사방으로 20px 이상 확장
            resizeHandleStyles={{
              bottom: { height: '30px', bottom: '-15px', cursor: 'ns-resize' },
              right: { width: '30px', right: '-15px', cursor: 'ew-resize' },
              bottomRight: { width: '40px', height: '40px', right: '-20px', bottom: '-20px', cursor: 'nwse-resize' }
            }}
        ...
        <div className="flex items-center gap-3 md:gap-6">
          ...
          <div className="flex gap-2 md:gap-4">
            {(['monitor', 'solution', 'blackboard'] as WindowType[]).map(id => (
              <button key={id} onClick={() => handleTaskbarToggle(id)} className={`${isNarrow ? 'w-12 h-12 rounded-2xl' : 'w-16 h-16 rounded-3xl'} flex items-center justify-center transition-all border ${windows[id].isOpen ? 'bg-white/10 border-white/20 text-white shadow-xl' : 'bg-white/5 border-white/5 text-slate-500 opacity-40 hover:opacity-100'}`}>
